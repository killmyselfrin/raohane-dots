...
Singleton {
    ...
    property bool raohaneLauncherOpen: false
    ...
    // Raohane-native names. The aliases keep compatibility providers working
    // while new UI code no longer depends on the old sidebar terminology.
    readonly property bool controlCenterOpen: root.sidebarRightOpen
    readonly property string controlCenterTargetOutput: root.sidebarRightTargetOutput
    ...
    readonly property string sidebarRightPresentationOutput:
        root.resolveOutputName(root.sidebarRightTargetOutput,
            Config.options?.sidebar?.screenList ?? [])
    readonly property string controlCenterPresentationOutput: root.sidebarRightPresentationOutput
