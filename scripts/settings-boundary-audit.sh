...
# Section extensions own their settings exclusively. Keep generic schemas free
# of bar controls that are rendered by Bar Studio to prevent duplicate UI.
bar_schema="$(awk '/^[[:space:]]*bar:[[:space:]]*\\{/{capture=1} capture{print} /^[[:space:]]*desktop:[[:space:]]*\\{/{exit}' "$registry")"
for key in barBottom barVertical barAutoHide barAutoHidePushWindows barShowOnSuper barShowOnSuperDelay barShowDate; do
  if grep -q "key:[[:space:]]*\"$key\"" <<<"$bar_schema"; then
    fail "Bar Studio-owned control leaked into generic bar schema: $key"
  fi
done
hyprland_schema="$(awk '/^[[:space:]]*hyprland:[[:space:]]*\\{/{capture=1} capture{print} /^[[:space:]]*services:[[:space:]]*\\{/{exit}' "$registry")"
if grep -q 'key:[[:space:]]*"barShowOnSuperDelay"' <<<"$hyprland_schema"; then
  fail 'Bar Studio-owned Super reveal delay leaked into Hyprland settings'
fi
if rg -q 'Bar pod size|styleValue\\("barScale"' "$catalog"; then
  fail 'Theme Studio reintroduced a duplicate bar size control'
fi
