...
required=(
  ...
  "$bar_studio" "$bar_appearance" "$bar_module_settings" "$quick_studio" "$quick_runtime" "$quick_registry" "$about" "$config" "$defaults" "$state" "$qmldir"
)
...
for symbol in 'RaohaneConfig\.barAutoHide' 'RaohaneConfig\.barShowOnSuperDelay'; do
  rg -q "$symbol" "$bar_appearance" || fail "Bar appearance lost single-owner control: $symbol"
done
rg -q 'RaohaneConfig\.barShowDate' "$bar_module_settings" || fail 'Clock module settings lost the single Show date owner'
if rg -n 'RaohaneConfig\.barShowDate' "$bar_appearance"; then
  fail 'Bar appearance duplicates Show date owned by Clock module settings'
fi
...
if rg -n '\bbarScale\b' "$config" modules/raohane/RaohaneBar.qml "$bar_studio" "$bar_appearance" "$bar_module_settings"; then
...
if rg -n '\.\./ii/settings/pages|modules/ii/settings/pages|^import qs$|^import qs\.services$|^import qs\.modules\.common|^import qs\.modules\.ii|\bGlobalStates\.' \
  "$content" "$navigation" "$header" "$registry" "$section_registry" "$router" "$section" "$control_row" "$preferences" "$language" "$settings" "$search" "$bar_module_settings" "$quick_studio"; then
