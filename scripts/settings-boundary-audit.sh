...
required=(
  ...
  "$bar_studio" "$bar_appearance" "$quick_studio" "$quick_runtime" "$quick_registry" "$about" "$config" "$defaults" "$state" "$qmldir"
)
...
for symbol in \
  'RaohaneConfig\.barAutoHide' 'RaohaneConfig\.barShowDate' 'RaohaneConfig\.barShowOnSuperDelay' \
  'RaohaneConfig\.barScreenList' 'RaohaneConfig\.barStylePreset' 'RaohaneConfig\.barOpacity'; do
  rg -q "$symbol" "$bar_appearance" || fail "Bar appearance editor lost owned setting: $symbol"
done
