...
bar_layout='modules/raohane/RaohaneBarLayoutEditor.qml'
...
required=(
  ...
  "$bar_studio" "$bar_appearance" "$bar_layout" "$quick_studio" "$quick_runtime" "$quick_registry" "$about" "$config" "$defaults" "$state" "$qmldir"
)
...
for contract in 'RaohaneConfig\.barAutoHide' 'RaohaneConfig\.barShowDate' 'RaohaneConfig\.barWorkspaceCount' 'RaohaneConfig\.barClock24Hour'; do
  rg -q "$contract" "$bar_appearance" || fail "Bar appearance editor lost native control: $contract"
done
...
# Single-source settings ownership. Bar behavior belongs only to Bar Studio;
# screen frame belongs to Appearance; dock geometry belongs to Bar & Dock.
for key in barBottom barVertical barAutoHide barAutoHidePushWindows barShowOnSuper barShowDate barShowOnSuperDelay; do
  if rg -q "type:[[:space:]]*\"(toggle|number|text)\",[[:space:]]*key:[[:space:]]*\"${key}\"" "$registry"; then
    fail "generic Settings duplicated Bar Studio control: $key"
  fi
done
if rg -q 'RaohaneConfig\.(frameEnabled|frameThickness)' "$bar_appearance"; then
  fail 'Bar Studio duplicated Appearance frame controls'
fi
if rg -q 'RaohaneConfig\.(dockHeight|dockIconSize)' "$catalog"; then
  fail 'Style Studio duplicated Dock geometry controls'
fi
if rg -q 'barScale' "$catalog" "$config" modules/raohane/RaohaneBar.qml; then
  fail 'legacy duplicate barScale control returned'
fi
for contract in \
...
printf 'settings-boundary-audit: all Settings routes share one animated registry/router/workspace, generic sections, reusable control rows and persisted studios/preferences pages\n'
 \
